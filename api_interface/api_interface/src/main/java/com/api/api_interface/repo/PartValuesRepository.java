...
public interface PartValuesRepository extends JpaRepository<PartValuesEntity,String>
{
    // @Query(value = "SELECT * FROM settings_machine_current WHERE machine_id = :ids or machine_id = :id", nativeQuery = true)
    // public List<PartValuesEntity> findProductsByIds(@Param("ids") String ids, @Param("id") String id);
}
